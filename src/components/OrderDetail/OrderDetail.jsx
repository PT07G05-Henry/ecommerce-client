...
const OrderDetail = (props) => {
    ...
    async function getOrder(id) {
        try {
            const orderDetail = await axios(`http://${process.env.REACT_APP_DEV_API || document.domain}/orders/${id}`);
    ...
    return(
        <div className="orderContainer">
            ...
            <div>
                ...
                <div>
                    <h4>{ order.user && order.user.first_name }, { order.user && order.user.last_name}</h4>
            ...
            <table>
                ...
                { order.products && order.products.map((product, index) => 
                    <tr key = { index }>
                        ...
                        <td>${ product.price }</td>
                        <td>${ product.price * product.orders_products.product_quantity} </td>
                    </tr>
                )}
            </table>
